...
export default function DashboardView({ onSelectCustomer, refreshTrigger }) {
  ...
  if (error) return <div className="p-6 bg-red-50 text-red-700 border border-red-200 rounded-xl text-center max-w-xl mx-auto">{error}</div>;
    ...
    <div className="space-y-6">
      <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-xs flex flex-col md:flex-row gap-4 justify-between items-center">
        ...
        <div className="flex flex-wrap gap-3 w-full md:w-auto items-center justify-end">
          ...
          <select value={genderFilter} onChange={(e) => setGenderFilter(e.target.value)} className="bg-white text-sm text-gray-700 border border-gray-200 rounded-xl px-3 py-2.5 focus:outline-none">
            <option value="All">All Genders</option>
            <option value="Male">Male</option>
            <option value="Female">Female</option>
          </select>
          <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="bg-white text-sm text-gray-700 border border-gray-200 rounded-xl px-3 py-2.5 focus:outline-none">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredCustomers.map((customer) => (
          <div key={customer._id} onClick={() => onSelectCustomer(customer._id)} className="bg-white rounded-2xl border border-gray-100 p-6 shadow-xs hover:shadow-md hover:border-tdc-logo-gold transition-all duration-200 cursor-pointer overflow-hidden flex flex-col group justify-between space-y-4 transform hover:-translate-y-0.5">
            <div className="flex justify-between items-start">
              <div>
                <h3 className="font-serif text-lg font-bold text-tdc-dark group-hover:text-tdc-logo-gold transition-colors">{customer.firstName} {customer.lastName}</h3>
              ...
              <span className={`text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full border ${getStatusStyle(customer.journeyStatus)}`}>{customer.journeyStatus}</span>
            </div>
            <div className="grid grid-cols-2 gap-y-3 gap-x-2 pt-3 border-t border-gray-50">
              <div className="flex items-center space-x-2 text-gray-600">
                <Calendar className="w-4 h-4 text-gray-400 shrink-0" />
                <span className="text-xs font-medium">{new Date().getFullYear() - new Date(customer.dateOfBirth).getFullYear()} Yrs Old</span>
              </div>
              <div className="flex items-center space-x-2 text-gray-600">
      ...
      {filteredCustomers.length === 0 && <div className="bg-white py-16 text-center border rounded-2xl text-gray-400 font-medium">No matches fit specified parameters.</div>}
    </div>
  );
}