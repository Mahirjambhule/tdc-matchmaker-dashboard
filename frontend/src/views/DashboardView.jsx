...
export default function DashboardView({ onSelectCustomer, refreshTrigger }) {
  ...
  useEffect(() => {
    ...
    if (statusFilter !== 'All') {
      result = result.filter(c => c.journeyStatus === statusFilter);
    }
  ...
  if (error) {
    return (
      <div className="p-6 bg-red-50 text-red-700 border border-red-200 rounded-xl text-center max-w-xl mx-auto">
        {error}
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-xs flex flex-col md:flex-row gap-4 justify-between items-center">
        <div className="relative w-full md:w-96">
          ...
          <input
            ...
            className="w-full bg-gray-50 text-sm rounded-xl pl-9 pr-4 py-2.5 border border-gray-200 focus:outline-none focus:border-tdc-logo-gold transition-all text-gray-800 placeholder-gray-400"
          />
        </div>
        ...
        <div className="flex flex-wrap gap-3 w-full md:w-auto items-center justify-end">
          ...
          <select
            ...
            className="bg-white text-sm text-gray-700 border border-gray-200 rounded-xl px-3 py-2.5 focus:outline-none"
          >
        ...
        {filteredCustomers.map((customer) => (
          ...
          >
            <div className="flex justify-between items-start">
              <div>
                <h3 className="font-serif text-lg font-bold text-tdc-dark group-hover:text-tdc-logo-gold transition-colors">
              ...
              <span className={`text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full border ${getStatusStyle(customer.journeyStatus)}`}>
            ...
            <div className="grid grid-cols-2 gap-y-3 gap-x-2 pt-3 border-t border-gray-50">
              <div className="flex items-center space-x-2 text-gray-600">
                ...
                <span className="text-xs font-medium">{calculateAge(customer.dateOfBirth)} Years Old</span>
              </div>
              <div className="flex items-center space-x-2 text-gray-600">
      ...
      {filteredCustomers.length === 0 && (
        <div className="bg-white py-16 text-center border rounded-2xl text-gray-400 font-medium">
          No matches fit specified parameters.
        </div>
      )}
    </div>
  );
